...
from runController import getMaps
...
if __name__ == "__main__":
    ...
    upAction = None
    downAction = None
    ranks = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4]
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            while rank_index < len(ranks):
                while trial_index < trial_for_each_rank:
                    ...
                    filtered_mean *= np.array(
                        [image_shape[1], image_shape[0], np.pi]
                    ).reshape((1, 3))
                    observations = (
                    ...
                    outputHotmap, actionMap = getMaps(
                        filtered_mean, covariances, image_shape
                    )
                    cv2.imshow(
                        "outputHotmap",
                        cv2.applyColorMap(
                            (outputHotmap / np.max(outputHotmap) * 255).astype(
                                np.uint8
                            ),
                            cv2.COLORMAP_HOT,
                        ),
                    ...
                    cv2.waitKey(1)
                    probability = ranks[rank_index] * np.max(outputHotmap)
                    dist = np.abs(outputHotmap - probability)
                    found_probability = dist.min()
                    loc = np.where(dist == found_probability)
                    if (
                        found_probability > 0.05 * np.max(outputHotmap)
                        or actionMap[loc[0][0], loc[1][0]] == -1
                    ):
                        print(
                        ...
                        break_flag = True
                        break
                    action = filtered_actions[actionMap[loc[0][0], loc[1][0]], :]
                    target = np.array(loc).flatten()
                    jointStateMsg.header.stamp = rospy.Time.now()
                    newPose[:4] = action[:4]
                    newPose[-3:] = action[-3:]
                    jointStateMsg.position = newPose
                    ...
                    if (
                        ...
                        errors.append(
                            np.array(
                                (
                                    np.abs(
                                        target[1] - tactile_state[0, 1] * image_shape[1]
                                    ),
                                    np.abs(
                                        target[0] - tactile_state[0, 2] * image_shape[0]
                                    ),
                                )
                            ).reshape(1, 2)
                        )
            ...
            if rank_index == len(ranks) and trial_index == trial_for_each_rank:
                ...
                errors = np.vstack(errors).reshape((len(ranks), trial_for_each_rank, 2))
                np.save(os.path.join("data", "prediction_errors.npy"), errors)
